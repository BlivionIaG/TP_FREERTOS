...
void tache_lecture_rapide(void *pvParameters){
		while(1){
			...
			if(semaphore){
				if(xSemaphoreTake(semaphore, (portTickType) (1000/portTick_RATE_MS)){
					unsigned int colis = 0;
					xQueueReceive(File_tapis_arrive, &colis, (1000/portTick_RATE_MS));
...
void affiche_message(char *texte, unsigned int colis){
	...
	if(semaphore){
		if(xSemaphoreTake(semaphore, (portTickType) (1000/portTick_RATE_MS)){
			printf("%s : compteur=%d B2=%d B1=%d B0=%d\n", texte, colis >> 3, (colis >> 2)&0x0001,   (colis >> 1)&0x0001, colis&0x0001);
